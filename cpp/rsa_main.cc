...
unsigned char *unbase64(const char *ascii, int len, int *flen) {
  ...
  if (len < 2) { // 2 accesses below would be OOB.
    ...
    puts("ERROR: You passed an invalid base64 string (too short). You get NULL "
         "back.");
    *flen = 0;
    return 0;
  }
...
RSA *loadPUBLICKeyFromString(const char *publicKeyStr) {
  ...
  // Since BIO_new_mem_buf will be READ ONLY, it's fine that publicKeyStr is
  // const.
  BIO *bio = BIO_new_mem_buf((void *)publicKeyStr,
                             -1); // -1: assume string is null terminated
...
RSA *loadPRIVATEKeyFromString(const char *privateKeyStr) {
  ...
  if (!rsaPrivKey)
    printf("ERROR: Could not load PRIVATE KEY!  PEM_read_bio_RSAPrivateKey "
           "FAILED: %s\n",
           ERR_error_string(ERR_get_error(), NULL));
...
unsigned char *rsaDecrypt(RSA *privKey, const unsigned char *encryptedData,
                          int *resultLen) {
  int rsaLen =
      RSA_size(privKey); // That's how many bytes the decrypted data would be
...
// You may need to encrypt several blocks of binary data (each has a maximum
// size limited by pubKey).  You shoudn't try to encrypt more than RSA_LEN(
// pubKey ) bytes into some packet. returns base64( rsa encrypt( <<binary data>>
// ) ) base64OfRsaEncrypted() base64StringOfRSAEncrypted rsaEncryptThenBase64
char *rsaEncryptThenBase64(RSA *pubKey, unsigned char *binaryData,
                           int binaryDataLen, int *outLen) {
  int encryptedDataLen;

  // RSA encryption with public key
  unsigned char *encrypted =
      rsaEncrypt(pubKey, binaryData, binaryDataLen, &encryptedDataLen);

  // To base 64
  int asciiBase64EncLen;
  char *asciiBase64Enc =
      base64(encrypted, encryptedDataLen, &asciiBase64EncLen);
...
unsigned char *rsaDecryptThisBase64(RSA *privKey, char *base64String,
                                    int *outLen) {
  int encBinLen;
  unsigned char *encBin =
      unbase64(base64String, (int)strlen(base64String), &encBinLen);
...
int main(int argc, const char *argv[]) {
  ...
  puts("We are going to: rsa_decrypt( unbase64( base64( rsa_encrypt( <<binary "
       "data>> ) ) ) )");
  // public key
  ...
  //-----END CERTIFICATE-----
  // YOUR PUBLIC KEY MUST CONTAIN NEWLINES.  If it doesn't (ie if you generated
  // it with something like ssh-keygen -t rsa -C "you@example.com" ) THEN YOU
  // MUST INSERT NEWLINES EVERY 64 CHRS (just line it up with how I have it here
  ...
  int dataSize =
      37; // 128 for NO PADDING, __ANY SIZE UNDER 128 B__ for RSA_PKCS1_PADDING
  ...
  char *rxOverHTTP =
      asciiB64E; // Simulate Internet connection by a pointer reference
  ...
  char *eav_encrypt =
      "So0NH7b2uJZ7KApmBq98CzV7MSUu1XmleaDIqYC1OzUfih5gCvsvd+jChSEiqal8E69ghl+"
      "MhQ6+yoOqqwwQ7JtvQqPJBWLE4aOdjO4codrmhH9zzgdwKuQHR7BUwjJSBjyZ85QeV51/"
      "ipY3gyqNnMSnsVfKxVv9xAZTE+fP5pg=";
  unsigned char *eavrBin =
      rsaDecryptThisBase64(eavprivKey, eav_encrypt, &rBinLen);
  printf("Decrypted %d bytes, the recovered data is:\n%.*s\n\n", rBinLen,
         rBinLen, eavrBin); // rBin is not necessarily NULL
