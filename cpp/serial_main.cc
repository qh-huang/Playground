...
public:
    ...
    vector<char> Read(int timeout_ms)
    {
        ...
        int maxfd = max(fd_, pipe_fd_[PIPE_READ_FD]);  /* maximum bit entry (fd) to test */
        vector <char> ret;
        ...
        if (select(maxfd + 1, &readfds, NULL, NULL, ptv) != -1) {
            if (FD_ISSET(pipe_fd_[PIPE_READ_FD], &readfds)) {
                ...
                while (nbytes = read(fd_, buf + total_bytes_read, BUFFER_SIZE - total_bytes_read) > 0) {
...
{
    // Tip: 
    // use 'socat pty,raw,echo=0,link=/home/user/dev/ttyS20 pty,raw,echo=0,link=/home/user/dev/ttyS21'
    // to create virtual serail ports for testing
    serial = new Serial();
