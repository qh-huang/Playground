...
TEST(BoostTest, LOG) {
    ...
    try
    {
        ...
        shared_ptr< std::ostream > strm_file(new std::ofstream("test.log"));
        if (!strm_file->good())
            throw std::runtime_error("Failed to open a text log file");

        // add streams to which logging records should be output
        shared_ptr< std::ostream > strm_console(&std::clog, boost::null_deleter());
        ...
        sink->locked_backend()->add_stream(strm_file);
        sink->locked_backend()->add_stream(strm_console);
